...
def load_images(img_path, ind=-1 , max_img_num = -1):
    if ind != -1: 
        X_test = np.zeros((1, IMG_HEIGHT, IMG_WIDTH, IMG_CHANEL), dtype=np.uint8)
        path_husos =  ind
        img = imread(path_husos)[:,:,:IMG_CHANEL]
        img =resize(img, (IMG_HEIGHT, IMG_WIDTH), mode='constant', preserve_range=True)
        X_test[0] = img
    else:
        test_ids = os.listdir(img_path)
        
        if max_img_num >= 0 and len(test_ids) > max_img_num:
            load_len = max_img_num
        else:
